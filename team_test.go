...
func TestGoquDBExecution(t *testing.T) {
	...
	db = goqu_example.InitLib("mysql", "grafana:password@tcp(localhost:3306)/grafana_tests?collation=utf8mb4_unicode_ci")
...
func TestSQLXDBExecution(t *testing.T) {
	...
	// TODO: the mysql test has some problem
	db = sqlx_example.InitLib("mysql", "grafana:password@tcp(localhost:3306)/grafana_tests?collation=utf8mb4_unicode_ci")
